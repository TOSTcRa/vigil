...
// filters for priority 3 (KERN_ERR) and 4 (KERN_WARNING) — most relevant for
// detecting anomalies like module load failures, memory violations, tainted kernel etc.
...
// called as the callback (or from it) for get_kernel_logs
// checks message text against known suspicious kernel patterns:
// - unsigned/out-of-tree module loads (rootkits, cheat drivers)
// - kernel taint (from loading unsigned modules)
// - segfaults and page faults (failed injection attempts)
// - /dev/mem or /dev/kmem access (direct physical memory r/w)
// - DMA/IOMMU errors (PCILeech-style hardware attacks)
// - usb device attach events (DMA attack dongles)
// - livepatch (runtime kernel code modification)
// - ptrace denials from LSMs (AppArmor/SELinux blocking debugger attach)
